...
def log_trade_operation(
    stock_code, operation, price, shares_held, current_balance, 
    total_assets, status='待执行', note=''
):
    """记录交易操作到CSV文件"""
    try:
        now = datetime.datetime.now()
        ...
        if "买入" in operation or "卖出" in operation:
            ...
            if "买入" in operation:
                amount = current_balance * percentage
                shares = amount / price if price > 0 else 0
            else:
                shares = shares_held * percentage
                amount = shares * price
        else:
            shares = 0
            amount = 0
        ...
        with open(TRADE_LOG_FILE, 'a', newline='', encoding='utf-8-sig') as f:
            ...
            writer.writerow([
                ...
                f"{price:.2f}", f"{shares:.2f}", f"{amount:.2f}", 
                f"{shares_held:.2f}", f"{current_balance:.2f}", 
                f"{total_assets:.2f}", status, note
            ])
...
def show_trade_summary():
    ...
    for i, op in enumerate(pending_ops, 1):
        ...
        lines.append(f"      状态: {op.get('操作状态', '未知')}")
...
def show_recent_trades(limit=10):
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            price = row.get('价格', '0.00')
            timestamp = row.get('时间戳', '未知')
            status = row.get('操作状态', '未知')
            
            icon = "🟢" if op_type == "买入" else "🔴" if op_type == "卖出" else "⚪"
            status_icon = "✅" if status == "已执行" else "⏳" if status == "待执行" else "❌"
            
            lines.append(f"   {icon} {op_type} {op_percentage} | 价格: {price} | {status_icon} {status}")
            lines.append(f"      时间: {timestamp}")
...
while True:
    try:
        ...
        if not df.empty and len(df) > 0:
            ...
            position_ratio = (position_value / total_assets * 100) if total_assets > 0 else 0  # 仓位比例
