"""
V5 实时预测脚本 - V7 模型 + LLM 市场情报参考 + 实时数据源
使用 V7 模型（126维价格序列）进行预测
LLM 市场情报作为决策参考信息显示，不输入到模型中

数据源优先级：
1. Tushare（如果配置了 token）- 支持实时数据
2. AkShare（免费备用）- 支持实时数据
3. baostock（最后备用）- 有延迟，不支持实时数据
...
STOCK_CODE = 'sh.600036'  # 股票代码（格式：sh.600036 或 sz.000858）
...
# ==================== 初始化 ====================
print("=" * 70)
print("V5 实时预测系统 - V7 模型 + LLM 市场情报参考 + 实时数据源")
print("=" * 70)
    ...
    for attempt in range(max_retries):
        try:
            ...
            if DATA_SOURCE == "tushare" and TUSHARE_AVAILABLE:
                df = fetch_tushare_5min(code_info, days=7)
                if df is not None and len(df) > 0:
                    return df
            ...
            if BAOSTOCK_AVAILABLE:
                df = fetch_baostock_5min(code_info, days=7)
                if df is not None and len(df) > 0:
                    return df
...
while True:
    try:
        ...
        # 获取数据
        df = fetch_data_with_retry(max_retries=3)
