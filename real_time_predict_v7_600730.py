  ...
  <div class="container">
    ...
    <div class="pnl-block">
      ...
      <div class="pnl-row" style="margin-top:12px; padding-top:12px; border-top:1px solid #e1e4e8;">
        <span class="pnl-label">累积盈亏：</span>
        <span class="pnl-value {{ pnl_class }}">{{ cumulative_pnl_display }}</span>
      </div>
      <div class="pnl-row">
        <span class="pnl-label">盈亏比例：</span>
        <span class="pnl-value {{ pnl_class }}">{{ pnl_percentage_display }}</span>
      </div>
    </div>
    ...
    def index():
        ...
        if request.method == "POST":
            try:
                ...
                shares_held = int(float(request.form.get("shares_held") or 0))  # 转换为整数（股数必须是整数）
        ...
        # 读取最新的模型预测建议（仅显示与当前持仓不一致的“买入/卖出”动作）
        suggestion_block = ""
        try:
            if os.path.exists(TRADE_LOG_FILE):
                ...
                if not df.empty:
                    # 过滤当前股票、预测状态
                    df_stock = df[(df["股票代码"] == str(data["stock_code"])) & (df["操作状态"] == "预测")].copy()
                    if len(df_stock) > 0:
                        df_stock = df_stock.sort_values("时间戳")
                        last = df_stock.iloc[-1]
                        op_type = str(last.get("操作类型", "未知"))
                        op_pct = str(last.get("操作比例", "0%"))
                        if op_type != "持有":
                            cur_price = float(last.get("当前价格", 0.0) or 0.0)

                            # 处理可能为 NaN 的字段，避免在页面上显示 'nan'
                            raw_buy = last.get("建议买入价格", "")
                            raw_sell = last.get("建议卖出价格", "")
                            raw_shares = last.get("预测数量", "")
                            raw_amt = last.get("预测金额", "")

                            s_buy = "" if pd.isna(raw_buy) else str(raw_buy)
                            s_sell = "" if pd.isna(raw_sell) else str(raw_sell)
                            pred_shares = "" if pd.isna(raw_shares) else str(raw_shares)
                            pred_amt = "" if pd.isna(raw_amt) else str(raw_amt)
                            ts = str(last.get("时间戳", "未知"))

                            # 根据当前持仓大致判断方向
                            shares_held_now = float(data.get("shares_held", 0.0))
                            last_price_now = float(data.get("last_price", cur_price) or 0.0)
                            cur_balance_now = float(data.get("current_balance", 0.0))
                            total_assets_now = cur_balance_now + shares_held_now * (last_price_now or 0.0)
                            pos_ratio = (shares_held_now * (last_price_now or 0.0) / total_assets_now * 100) if total_assets_now > 0 else 0.0

                            if op_type == "买入":
                                action_desc = "模型建议相对于当前持仓：加仓 / 建仓"
                            elif op_type == "卖出":
                                action_desc = "模型建议相对于当前持仓：减仓 / 平仓"
                            else:
                                action_desc = "模型当前建议：持有"

                            price_line = f"当前价格：{cur_price:.2f} 元"
                            if s_buy:
                                price_line += f" ｜ 建议买入价：{s_buy} 元"
                            if s_sell:
                                price_line += f" ｜ 建议卖出价：{s_sell} 元"

                            # 根据当前持仓状态实时重新计算建议数量（而不是使用CSV中的旧值）
                            size_line = ""
                            if op_type == "买入" and s_buy:
                                # 使用当前可用资金重新计算建议数量
                                available_balance = max(0.0, cur_balance_now)
                                # 如果持仓为0且可用资金异常，使用初始资金
                                if shares_held_now <= 0 and (available_balance <= 0 or available_balance < 0.01):
                                    available_balance = float(data.get("initial_balance", 0.0)) or total_assets_now
                                
                                try:
                                    buy_price = float(s_buy)
                                    op_pct_val = float(op_pct.replace("%", ""))
                                    predicted_amount = available_balance * (op_pct_val / 100.0)
                                    if buy_price > 0 and predicted_amount > 0:
                                        recalc_shares = predicted_amount / buy_price
                                        recalc_amt = predicted_amount
                                        size_line = f"建议数量：{recalc_shares:.2f} 股 ｜ 约 {recalc_amt:.2f} 元"
                                    else:
                                        size_line = f"建议数量：0.0 股 ｜ 约 0.0 元"
                                except:
                                    # 如果计算失败，使用CSV中的旧值
                                    if pred_shares and pred_amt:
                                        size_line = f"建议数量：{pred_shares} 股 ｜ 约 {pred_amt} 元"
                            elif op_type == "卖出" and s_sell:
                                # 卖出操作：使用当前持仓数量计算
                                try:
                                    sell_price = float(s_sell)
                                    op_pct_val = float(op_pct.replace("%", ""))
                                    predicted_shares = shares_held_now * (op_pct_val / 100.0)
                                    predicted_amount = predicted_shares * sell_price
                                    if predicted_shares > 0 and predicted_amount > 0:
                                        size_line = f"建议数量：{predicted_shares:.2f} 股 ｜ 约 {predicted_amount:.2f} 元"
                                    else:
                                        size_line = f"建议数量：0.0 股 ｜ 约 0.0 元"
                                except:
                                    # 如果计算失败，使用CSV中的旧值
                            ...
                            pos_line = f"当前持仓：{shares_held_now:.2f} 股（仓位约 {pos_ratio:.1f}%）"

                            exec_note = ""
                            # 方案A：如果模型建议买入但当前已接近满仓，说明执行层保持不变
                            if (op_type == "买入") and (pos_ratio >= 95 or float(data.get("current_balance", 0.0)) < 100):
                                exec_note = "（执行层说明：当前已接近满仓或可用资金不足，本轮不再加仓，实际操作视为“持有”）"

                            suggestion_block = f"""
    <div style="margin-top:18px;padding:12px 14px;border-radius:10px;background:#f0f7ff;border:1px solid #d0e2ff;">
      <div style="font-weight:600;font-size:14px;margin-bottom:4px;">📊 当前模型交易建议</div>
      <div style="font-size:12px;color:#666;margin-bottom:4px;">时间：{ts}</div>
      <div style="font-size:13px;margin-bottom:4px;">模型动作：<strong>{op_type} {op_pct}</strong></div>
      <div style="font-size:13px;margin-bottom:4px;">{price_line}</div>
      {"<div style='font-size:13px;margin-bottom:4px;'>" + size_line + "</div>" if size_line else ""}
      <div style="font-size:13px;margin-bottom:4px;">{pos_line}</div>
      <div style="font-size:13px;color:#005fa3;">{action_desc}{exec_note}</div>
    </div>
"""
        except Exception as e:
            # 出错时忽略建议展示，保持编辑器可用
        ...
        # 格式化显示
        initial_balance_display = f"{initial_balance_val:,.2f}"
        position_value_display = f"{position_value:,.2f}"
        current_balance_display = f"{current_balance_val:,.2f}"
        total_assets_display = f"{total_assets:,.2f}"
        cumulative_pnl_display = f"{pnl_sign}{cumulative_pnl:,.2f} 元"
        pnl_percentage_display = f"{pnl_sign}{pnl_percentage:.2f}%"

        return app.response_class(
            TEMPLATE.replace("{{ host }}", WEB_EDITOR_HOST).replace("{{ port }}", str(WEB_EDITOR_PORT))
                    .replace("{{ stock_code }}", str(data["stock_code"]))
                    .replace("{{ shares_held }}", str(int(data["shares_held"])))  # 显示为整数
                    ...
                    .replace("{{ current_balance_display }}", current_balance_display)
                    .replace("{{ total_assets_display }}", total_assets_display)
                    .replace("{{ cumulative_pnl_display }}", cumulative_pnl_display)
                    .replace("{{ pnl_percentage_display }}", pnl_percentage_display)
    ...
    try:
        ...
        if "买入" in operation or "卖出" in operation:
            ...
            if "买入" in operation:
                ...
                # 计算可用资金：如果持仓为0且可用资金为0或负数，使用初始资金
                if shares_held <= 0 and (current_balance <= 0 or current_balance < 0.01):
                    # 持仓为0时，如果可用资金异常，使用初始资金（如果提供了）
                    if initial_balance and initial_balance > 0:
                        available_balance = initial_balance
                    else:
                        # 如果没有提供初始资金，尝试从总资产推断
                        # 如果总资产接近初始资金，使用总资产
                        available_balance = max(0.0, total_assets) if total_assets > 0 else max(0.0, current_balance)
                else:
                    # 正常情况：确保可用资金不为负数
                    available_balance = max(0.0, current_balance)
                # 使用建议买入价格计算预测数量（更准确）
                predicted_amount = available_balance * percentage
                # 使用建议买入价格计算数量，确保使用正数价格
                price_for_calc = suggested_buy_price if suggested_buy_price and suggested_buy_price > 0 else current_price
                if price_for_calc > 0 and predicted_amount > 0:
                    predicted_shares = predicted_amount / price_for_calc
                else:
                    predicted_shares = 0.0
                # 确保数量不为负数
                predicted_shares = max(0.0, predicted_shares)
            else:
                # 如果没有提供建议卖出价格，使用当前价格
                ...
                price_for_calc = suggested_sell_price if suggested_sell_price and suggested_sell_price > 0 else current_price
                predicted_amount = predicted_shares * price_for_calc
                # 确保金额不为负数
...
if portfolio_state:
    ...
    print(f"   可用资金: {current_balance:.2f} 元（已根据持仓自动计算）")
    print()
else:
    # 使用默认初始状态
